...
use App\Jobs\FollowOrderHandel;
...
//            }
     ...
     * @return bool
     */
    public static function leverClose($lever_transaction, $is_follow = false)
    {
        try {
            DB::beginTransaction();
            ...
            //处理跟随订单，这里用个判断防止无限递归
            if($is_follow === false) {
                FollowOrderHandel::dispatch($lever_transaction->id)->onQueue('FollowOrderHandel');
            }
            return true;
        } catch (\Exception $e) {
            DB::rollBack();
