...
class LegalDealController extends Controller
{
     ...
     public function getMoneyType(){
        $type = LegalDealSend::MONEY_TYPE;
        $data = [];
        foreach($type as $k => $v){
            $data[$k] = ['name' => $v , 'rate' => 1];
        }
    ...
    public function postSend(Request $request)
    {
        ...
        try {
            ...
            if ($type == 'sell') {   
                ...
                $res2 = change_wallet_balance($user_wallet, 1, $total_number, AccountLog::LEGAL_DEAL_SEND_SELL, '商家发布出售,锁定增加',1);
            ...
            $legal_deal_send->money_type = $request->get('money_type')??1;
    ...
    public function doDeal(Request $request)
    {
        ...
        try {
            ...
            if ($legal_deal_send->type == 'buy') { //求购
                ...
                $result1 = change_wallet_balance($users_wallet, 1, $number, AccountLog::LEGAL_DEAL_USER_SELL, '出售给商家法币:增加锁定', true);
                if ($result1 !== true) {
    ...
    public function doSure(Request $request)
    {
        ...
        try {
            ...
            if (bc_comp($seller->lock_seller_balance, $legal_deal->number) < 0) {
                throw new \Exception('对不起，您的商家锁定余额不足,当前余额:' . $seller->lock_seller_balance);
            }
        ...
        try {
            ...
            if ($legal_send->type == 'sell') {   //如果商家发布出售信息
                if ($seller->lock_seller_balance < $legal_send->surplus_number){
                    DB::rollback();
                    return $this->error('对不起，您的商家账户锁定资金不足');
                }
                ...
                if ($res1 !== true) {
                    throw new \Exception('撤回失败:减少锁定资金失败');
                }
    ...
    public function errorSend(Request $request)
    {
        ...
        try {
            ...
            if ($legal_send->type == 'sell') {   //如果商家发布出售信息
                ...
                if ($res1 !== true) {
                    throw new \Exception('处理异常失败:减少锁定资金失败');
                }
