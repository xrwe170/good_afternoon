<template>
	<view class="d-flex align-items-center u-subsection" :style="[subsectionStyle]">
		<view class="navs" :style="[itemStyle(index),{'--after-width': afterWidths}]" @tap="click(index)" :class="[noBorderRight(index), 'navs-' + index, currentIndex == index ? 'navs-curr' : '']"
	...
	export default {
		...
		data() {
			return {
				...
				afterWidths: uni.upx2px(this.afterWidth)+"px",
	...
	.navs{
		...
		&.navs-curr {
			...
			&::after {
			    ...
			    display: block;
				margin-top: 15rpx;
				// width: $u-navs-text-width;
				width: var(--after-width);
				height: 4rpx;
			    background: #2e5cd1;
				border-radius: 347rpx;
				margin-left: calc((100% - var(--after-width)) / 2);
			}
		}
	}
